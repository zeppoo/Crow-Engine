 ...
 *                                         *
 *******************************************/
void RecreateSwapchain(VkDevice device, VkSurfaceKHR surface, VkSwapchainKHR oldSwapchain,
                       VkExtent2D newExtent); // Recreates the swapchain during resizing or format changes.
void PresentSwapchainImage(VkQueue queue, VkSwapchainKHR swapchain, uint32_t imageIndex,
                           VkSemaphore waitSemaphore); // Submits the image to the presentation engine.
...
void BeginRenderPass(VkCommandBuffer commandBuffer, VkRenderPass renderPass, VkFramebuffer framebuffer, const VkRect2D &renderArea,
                     const VkClearValue *clearValues, uint32_t clearValueCount); // Begins a render pass.
void EndRenderPass(VkCommandBuffer commandBuffer); // Ends the currently active render pass.
void UpdateRenderPassAttachments(VkRenderPass renderPass,
                                 const std::vector<VkAttachmentDescription> &attachments); // Updates the attachments of an existing render pass.
void SetRenderPassDependency(VkRenderPass renderPass,
                             const VkSubpassDependency &dependency); // Sets a subpass dependency for synchronization between subpasses.
...
void SetBlendState(bool enable, VkBlendFactor srcFactor, VkBlendFactor dstFactor,
                   VkBlendOp blendOp); // Configures blending state for color attachment.
...
void RebuildPipeline(VkRenderPass renderPass, VkPipelineLayout pipelineLayout,
                     const PipelineConfig &config); // Rebuilds the graphics pipeline with updated configurations.
void SwitchToWireframeMode(); // Switches the polygon mode to wireframe.
void SetRasterizationState(
    const VkPipelineRasterizationStateCreateInfo &rasterizationState); // Updates the rasterization state (e.g., depth bias, polygon mode).
...
void PushConstants(VkPipelineLayout pipelineLayout, VkShaderStageFlags stageFlags, uint32_t offset, uint32_t size,
                   const void *data); // Updates push constants for a specific shader stage.
void BindTextureToDescriptor(VkDescriptorSet descriptorSet, uint32_t binding, VkImageView imageView,
                             VkSampler sampler); // Binds a texture to a descriptor set at a specific binding.
