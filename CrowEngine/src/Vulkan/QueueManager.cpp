...
void QueueManager::FindQueueFamilies(VkPhysicalDevice &physicDevice, VkSurfaceKHR &surface)
{
  ...
  AssignQueuesToQueueFamilies(physicDevice, surface, VK_QUEUE_GRAPHICS_BIT, currentQueueFamilies, queueFamilyProperties, presentQueues,
                              settings::getQueueConfig().presentQueuesCount, true);
  AssignQueuesToQueueFamilies(physicDevice, surface, VK_QUEUE_GRAPHICS_BIT, currentQueueFamilies, queueFamilyProperties, graphicsQueues,
                              settings::getQueueConfig().graphicsQueuesCount, false);
  AssignQueuesToQueueFamilies(physicDevice, surface, VK_QUEUE_COMPUTE_BIT, currentQueueFamilies, queueFamilyProperties, computeQueues,
                              settings::getQueueConfig().computeQueuesCount, false);
  AssignQueuesToQueueFamilies(physicDevice, surface, VK_QUEUE_TRANSFER_BIT, currentQueueFamilies, queueFamilyProperties, transferQueues,
                              settings::getQueueConfig().transferQueuesCount, false);
  CleanupEmptyFamilies(currentQueueFamilies);

  queueFamilies = currentQueueFamilies; // Allocate to prevent memory fragmentation
}

void QueueManager::AssignQueuesToQueueFamilies(VkPhysicalDevice &physicDevice, VkSurfaceKHR &surface, VkQueueFlagBits flagBit,
                                               std::vector<QueueFamily> &currentQueueFamilies,
                                               ...
                                               int queueCount, bool isPresentQueue)
{
  ...
  for (int i = 0; i < queueCount; i++) { // Iterate for the amount of queues we're looking to allocate
    for (int j = 0; j < queueFamiliesProperties.size(); j++) { // Iterate over every family we found
      int supportedQueueCount =
          queueFamiliesProperties[j].queueCount - currentQueueFamilies[j].queueCount; // Check if current family has enough queues left to assign
      if (supportedQueueCount <= 0)
        continue; // Skip if no queues are available
    ...
    QueueData data = CreateQueueData(currentQueueFamilies[bestQueueFamilyindex], queueFamiliesProperties[bestQueueFamilyindex].queueCount);
...
{
  return ((flags & VK_QUEUE_GRAPHICS_BIT) ? 1 : 0) + ((flags & VK_QUEUE_COMPUTE_BIT) ? 1 : 0) + ((flags & VK_QUEUE_TRANSFER_BIT) ? 1 : 0) +
